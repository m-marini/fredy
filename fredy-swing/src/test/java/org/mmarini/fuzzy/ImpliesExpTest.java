...
public class ImpliesExpTest {
	...
	public void testExecute(FuzzyBoolean value1, FuzzyBoolean value2) {
		ImpliesExp exp = new ImpliesExp(new ConstantExp(value1),
				new ConstantExp(value2));
		exp.execute(mock);
		assertThat(mock, hasProperty("size", equalTo(1)));
		assertThat(mock, hasProperty("value", equalTo(value1.implies(value2))));
	}
	...
	public void testToString(FuzzyBoolean value1, FuzzyBoolean value2) {
		ImpliesExp exp = new ImpliesExp(new ConstantExp(value1),
				new ConstantExp(value2));
		assertThat(exp.toString(), equalTo("implies[" + value1 + ", " + value2
				+ "]"));
	}
}
