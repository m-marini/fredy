...
public class VeryExpTest {
	...
	public void testExecute(FuzzyBoolean value) {
		VeryExp exp = new VeryExp(new ConstantExp(value));
		exp.execute(mock);
		assertThat(mock, hasProperty("size", equalTo(1)));
		assertThat(mock, hasProperty("value", equalTo(value.very())));
	}

	@Theory
	public void testToString(FuzzyBoolean value) {
		VeryExp exp = new VeryExp(new ConstantExp(value));
		assertThat(exp.toString(), equalTo("very(" + value + ")"));
	}
}
